            ...
            <div class="card-body py-5">
                <div class="row g-3 align-items-end">
                    <div class="col-sm-4">
                        ...
                        <input v-model="filter.search" type="text" class="form-control form-control-sm" placeholder="Cari poli, dokter..." @input="loadData" />
                    </div>
                    <div class="col-sm-4">
                ...
                <table v-else class="table align-middle table-row-dashed fs-6 gy-5">
                    ...
                    <tbody class="fw-semibold text-gray-600">
                        <tr v-for="(item, i) in riwayat" :key="item.id">
                            ...
                            <td>{{ item.dokter }}</td>
                            <td>{{ item.diagnosa ?? '-' }}</td>
                            <td class="text-center"><span class="badge badge-light-success">Selesai</span></td>
                            <td class="text-end">
                                <router-link :to="`/pasien/riwayat/${item.id}`" class="btn btn-sm btn-icon btn-light btn-active-primary">
