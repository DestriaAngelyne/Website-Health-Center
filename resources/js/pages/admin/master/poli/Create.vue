  ...
  <div class="page">
    <div class="topbar">
      <div class="topbar-left">
        <router-link to="/admin/master/poli" class="back-btn">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m15 18-6-6 6-6"/></svg>
        </router-link>
        <div>
          <h1 class="page-title">Tambah Poli</h1>
          <p class="page-sub">Tambah poliklinik baru</p>
        </div>
      </div>
    ...
    <div class="card">
      <form @submit.prevent="submit">
        <div class="section">
          ...
          <div class="grid2">
            <div class="field">
              <label class="label">Kode Poli <span class="req">*</span></label>
              <input v-model="form.kode" type="text" class="input" :class="{ err: errors.kode }" placeholder="Contoh: POL001" />
              <p v-if="errors.kode" class="err-msg">{{ errors.kode[0] }}</p>
            </div>
            <div class="field">
              <label class="label">Nama Poli <span class="req">*</span></label>
              <input v-model="form.nama_poli" type="text" class="input" :class="{ err: errors.nama_poli }" placeholder="Contoh: Poli Umum" />
              <p v-if="errors.nama_poli" class="err-msg">{{ errors.nama_poli[0] }}</p>
            </div>
          </div>
          <div class="grid2">
            <div class="field">
              <label class="label">Singkatan</label>
              <input v-model="form.singkatan" type="text" class="input" placeholder="Contoh: POLUM" />
            </div>
            <div class="field">
          ...
          <div class="field">
            ...
            <textarea v-model="form.keterangan" class="input textarea" rows="3" placeholder="Keterangan tambahan (opsional)"></textarea>
          </div>
        </div>
        <div class="footer">
...
async function submit() {
  ...
  try { await api.post('/admin/poli', form.value); router.push('/admin/master/poli') }
...
.page-sub{font-size:.9rem;color:var(--text2);margin:.15rem 0 0}
...
.sec-title{font-size:.72rem;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.1em;margin:0 0 1.2rem}
...
.input::placeholder{color:var(--text3)}.input.err{border-color:#ef4444}
.textarea{resize:vertical;min-height:80px}
.err-msg{font-size:.8rem;color:#ef4444;margin:0}
