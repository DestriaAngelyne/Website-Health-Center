  ...
  <div class="page">
    <div class="topbar">
      <div class="topbar-left">
        ...
        <div><h1 class="page-title">Edit Dokter</h1><p class="page-sub">Ubah data dokter</p></div>
      </div>
    ...
    <div v-else class="card">
      <form @submit.prevent="submit">
        <div class="section">
          ...
          <div class="grid2">
            <div class="field"><label class="label">NIP</label><input v-model="form.nip" type="text" class="input" /></div>
            <div class="field"><label class="label">STR Number</label><input v-model="form.str_number" type="text" class="input" /></div>
          </div>
          <div class="grid2">
            <div class="field"><label class="label">Nama Lengkap <span class="req">*</span></label><input v-model="form.nama" type="text" class="input" :class="{ err: errors.nama }" /><p v-if="errors.nama" class="err-msg">{{ errors.nama[0] }}</p></div>
            <div class="field"><label class="label">Spesialis</label><input v-model="form.spesialis" type="text" class="input" /></div>
          </div>
          <div class="grid2">
            ...
            <div class="field"><label class="label">No HP</label><input v-model="form.no_hp" type="text" class="input" /></div>
          </div>
        ...
        <div class="footer">
          ...
          <button type="submit" class="btn-save" :disabled="saving"><span v-if="saving" class="btn-spin"></span>{{ saving ? 'Menyimpan...' : 'Update Dokter' }}</button>
        </div>
      </form>
    </div>
  </div>
...
const form = ref({ nip: '', nama: '', spesialis: '', poli_id: '', no_hp: '', str_number: '', is_active: true })
const errors = ref({}), saving = ref(false), loadingData = ref(true), poliList = ref([])
onMounted(async () => {
  try {
    const [pr, dr] = await Promise.all([api.get('/admin/poli', { params: { per_page: 100 } }), api.get(`/admin/dokter/${route.params.id}`)])
    poliList.value = pr.data.data; const d = dr.data.data
    form.value = { nip: d.nip ?? '', nama: d.nama, spesialis: d.spesialis ?? '', poli_id: d.poli_id, no_hp: d.no_hp ?? '', str_number: d.str_number ?? '', is_active: d.is_active }
  } catch { alert('Gagal memuat data') } finally { loadingData.value = false }
})
async function submit() {
  saving.value = true; errors.value = {}
  try { await api.put(`/admin/dokter/${route.params.id}`, form.value); router.push('/admin/master/dokter') }
