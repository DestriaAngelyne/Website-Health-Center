  ...
  <div class="page">
    <div class="topbar">
      <div class="topbar-left">
        ...
        <div><h1 class="page-title">Edit Jadwal</h1><p class="page-sub">Ubah jadwal praktik dokter</p></div>
      </div>
    </div>
    ...
    <div v-else class="card">
      <form @submit.prevent="submit">
        <div class="section">
          ...
          <div class="grid2">
            <div class="field"><label class="label">Dokter <span class="req">*</span></label><select v-model="form.dokter_id" class="input"><option value="">— Pilih Dokter —</option><option v-for="d in dokterList" :key="d.id" :value="d.id">{{ d.nama }} ({{ d.poli?.nama_poli }})</option></select></div>
            <div class="field"><label class="label">Hari <span class="req">*</span></label><select v-model="form.hari" class="input"><option value="">— Pilih Hari —</option><option v-for="h in hariList" :key="h" :value="h">{{ h }}</option></select></div>
          </div>
          <div class="grid3">
            <div class="field"><label class="label">Jam Buka</label><input v-model="form.jam_buka" type="time" class="input" /></div>
            <div class="field"><label class="label">Jam Tutup</label><input v-model="form.jam_tutup" type="time" class="input" /></div>
            <div class="field"><label class="label">Kuota</label><input v-model.number="form.kuota" type="number" min="1" class="input" /></div>
          </div>
        ...
        <div class="footer">
          ...
          <button type="submit" class="btn-save" :disabled="saving"><span v-if="saving" class="btn-spin"></span>{{ saving ? 'Menyimpan...' : 'Update Jadwal' }}</button>
        </div>
      </form>
    </div>
  </div>
...
const errors = ref({}), saving = ref(false), loadingData = ref(true), dokterList = ref([])
onMounted(async () => {
  try {
    const [dr, jr] = await Promise.all([api.get('/admin/dokter', { params: { per_page: 100 } }), api.get(`/admin/jadwal/${route.params.id}`)])
    dokterList.value = dr.data.data; const d = jr.data.data
    form.value = { dokter_id: d.dokter_id, hari: d.hari, jam_buka: d.jam_buka, jam_tutup: d.jam_tutup, kuota: d.kuota, is_active: d.is_active }
  } catch { alert('Gagal memuat data') } finally { loadingData.value = false }
})
async function submit() {
  saving.value = true; errors.value = {}
  try { await api.put(`/admin/jadwal/${route.params.id}`, form.value); router.push('/admin/master/jadwal') }
