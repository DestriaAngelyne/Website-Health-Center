    ...
    <div class="form-card">
      ...
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <input v-model.number="form.stok" type="number" min="0" class="form-input" :class="{ error: errors.stok }" placeholder="0"/>
          <span v-if="errors.stok" class="err-msg">{{ errors.stok[0] }}</span>
        </div>

        <div class="form-group">
          <label class="form-label required">Stok Minimum
            <span class="label-hint">( peringatan kritis )</span>
          </label>
          <input v-model.number="form.stok_minimum" type="number" min="0" class="form-input" :class="{ error: errors.stok_minimum }" placeholder="10"/>
        ...
        <div class="form-group">
          ...
          <div class="input-prefix">
            ...
            <input v-model.number="form.harga" type="number" min="0" class="form-input prefixed" :class="{ error: errors.harga }" placeholder="0"/>
          </div>
      ...
      <div class="form-actions">
        ...
        <button @click="submit" class="btn-submit" :disabled="loading">
          <svg v-if="!loading" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2.5" style="width:15px;height:15px"><path d="M10 4v12M4 10h12"/></svg>
...
const form = ref({
  nama_obat: '', kategori: '', satuan: '', stok: 0,
  stok_minimum: 10, harga: 0, deskripsi: '', is_active: true,
})
...
async function submit() {
  ...
  try {
    await api.post('/admin/master/obat', form.value)
    router.push({ path: '/admin/master/obat', query: { success: 'Obat berhasil ditambahkan.' } })
  } catch (e) {
    ...
    else { alertMsg.value = 'Gagal menyimpan obat. Coba lagi.'; alertType.value = 'error' }
...
.label-hint{font-size:.75rem;font-weight:400;color:var(--text3);margin-left:4px}
